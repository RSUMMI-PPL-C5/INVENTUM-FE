...
export function FilterDialog({
	...
	const [filters, setFilters] = useState<SparepartFilters>({
		...
		purchaseDateEnd: currentFilters?.get("purchaseDateEnd")
			...
			: null,
		priceMin: currentFilters?.get("priceMin")
			? Number.parseFloat(currentFilters.get("priceMin") as string)
			: null,
		priceMax: currentFilters?.get("priceMax")
			? Number.parseFloat(currentFilters.get("priceMax") as string)
			: null,
		createdOnStart: currentFilters?.get("createdOnStart")
			? new Date(currentFilters.get("createdOnStart") as string)
			: null,
		createdOnEnd: currentFilters?.get("createdOnEnd")
			? new Date(currentFilters.get("createdOnEnd") as string)
			: null,
		...
		setFilters((prev) => {
			...
			if (date) {
				...
				if (
					...
					prev.purchaseDateEnd &&
					date > prev.purchaseDateEnd
				) {
	...
	const handleApplyFilter = () => {
		...
		if (filters.purchaseDateStart) {
			filterParams.purchaseDateStart =
				filters.purchaseDateStart.toISOString();
		}
		if (filters.purchaseDateEnd) {
			filterParams.purchaseDateEnd =
				filters.purchaseDateEnd.toISOString();
		}
		...
		if (filters.modifiedOnStart) {
			filterParams.modifiedOnStart =
				filters.modifiedOnStart.toISOString();
		}
			...
			>
				<DialogContent className="w-full max-w-[90vw] max-h-[90vh] overflow-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Pilih filter untuk menyaring daftar suku cadang
						</DialogDescription>
					</DialogHeader>
					...
					<div className="flex flex-col md:flex-row gap-8 py-4">
							...
							<div className="space-y-2">
								...
								<div className="flex flex-col sm:flex-row gap-4">
									<div className="flex-1">
										...
										<div className="relative">
											<span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs-semibold text-gray-400/80">
												Rp
											</span>
											<Input
												id="priceMin"
						...
						<div className="flex flex-col gap-8">
							...
							<div className="space-y-2">
								...
								<div className="flex flex-col sm:flex-row gap-4">
									<div className="flex-1">
										<Label htmlFor="purchaseDateStart">
											Dari
										</Label>
										<Popover>
