...
export default function UserEdit() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!divisionsResponse.ok) {
        ...
        throw new Error(`Gagal mengambil data divisi: ${errorData.message ?? 'Error tidak diketahui'}`);
      }
      ...
      if (!userResponse.ok) {
        ...
        throw new Error(`Gagal mengambil data pengguna: ${errorData.message ?? 'Error tidak diketahui'}`);
      }
  ...
  // Panggil fetchData hanya sekali saat komponen dimount atau userId berubah
  useEffect(() => {
    if (userId) {
      fetchData();
    }
  ...
  async function updateUser(data: z.infer<typeof formSchema>) {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : 'Gagal memperbarui data pengguna');
    } finally {
      setLoading(false);
    }
  }
  ...
  // Jika ada kesalahan saat fetch, tampilkan pesan error
