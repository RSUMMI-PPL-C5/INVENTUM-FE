...
export default function LoginModule() {
	...
	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		try {
			...
			Cookies.set("accessToken", responseData.data.user.token, {
				expires: 7,
			});
		...
		<div className="min-h-screen flex items-center justify-center px-4 py-8">
			<div className="flex flex-col items-center gap-6 md:gap-8 w-full max-w-md">
				<Image
					...
					priority
				/>
				<div className="bg-white w-full gap-6 md:gap-8 px-6 md:px-12 py-6 flex flex-col items-center justify-center shadow-[0px_0px_100px_0px_rgba(0,0,0,0.10)] text-m-medium font-plus-jakarta-sans rounded-lg">
					<div className="flex flex-col items-center justify-center text-center">
						<span className="text-header-h4 md:text-header-h3 font-poppins text-primary-solid">
							INVENTUM
						</span>
						<span className="text-base">
							Inventaris Terpadu RS UMMI
						</span>
					</div>
					<Form {...form}>
						<form
