...
const formSchema = z.object({
	actionPerformed: z
		.string()
		.min(1, { message: "Tindakan yang dilakukan wajib diisi" }),
	...
	calibrationMethod: z
		.string()
		.min(1, { message: "Metode kalibrasi wajib diisi" }),
...
export default function CalibrationHistoryCreate() {
		...
		async function fetchMedicalEquipment() {
			try {
				...
				const response = await fetch(
					`${process.env.NEXT_PUBLIC_API_URL}/medical-equipment/${medicalEquipmentId}`,
					{
						method: "GET",
						headers: {
	...
	async function createCalibrationHistory(data: z.infer<typeof formSchema>) {
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/medical-equipment/${medicalEquipmentId}/calibration-history`,
				{
					method: "POST",
					headers: {
						"Content-Type": "application/json",
			...
			toast.error(
				error instanceof Error
					? error.message
					: "Error membuat riwayat kalibrasi"
			);
		}
	}
				...
				<div className="flex flex-col">
					...
					{equipment && (
						<span className="text-muted-foreground">
							{equipment.name} - {equipment.brandName}{" "}
							{equipment.modelName}
						</span>
					)}
				</div>
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormControl>
									<Input
										{...field}
										placeholder="Masukkan metode kalibrasi"
									/>
								</FormControl>
								<FormMessage />
							</FormItem>
