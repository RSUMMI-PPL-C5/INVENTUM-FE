...
const formSchema = z.object({
  ...
  calibrationDate: z.date({
    required_error: "Tanggal kalibrasi wajib diisi",
  }),
  calibrationMethod: z.string().min(1, { message: "Metode kalibrasi wajib diisi" }),
  nextCalibrationDue: z.date().optional(),
})
...
export default function CalibrationHistoryCreate() {
  ...
  async function createCalibrationHistory(data: z.infer<typeof formSchema>) {
    try {
      const token = Cookies.get("accessToken")
      ...
      const payload = {
        ...
        result: data.result,
        calibrationDate: data.calibrationDate.toISOString(),
      ...
      if (!response.ok) {
        const result = await response.json()
        toast.error("Error membuat riwayat kalibrasi", result.message)
        return
      }
      ...
      toast.error(error instanceof Error ? error.message : "Error membuat riwayat kalibrasi")
    }
  }
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      await createCalibrationHistory(values)
      toast.success("Riwayat kalibrasi berhasil dibuat")
      router.push(`/dashboard/medical-equipment/${medicalEquipmentId}`)
    } catch (error) {
      console.error("Failed to create calibration history:", error)
      setError("Gagal membuat riwayat kalibrasi. Silakan coba lagi.")
    } finally {
      setLoading(false)
    }
  }
    ...
    <>
      <div className="flex flex-col items-start gap-4">
        ...
        <div className="flex flex-col">
          <span className="text-header-h5 font-bold font-poppins">Tambah Riwayat Kalibrasi</span>
          {equipment && (
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-4">
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea
                    {...field}
                    placeholder="Jelaskan tindakan kalibrasi yang dilakukan"
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>Metode Kalibrasi</FormLabel>
                <FormControl>
                  <Input {...field} placeholder="Masukkan metode kalibrasi" />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                <FormLabel>Tanggal Kalibrasi</FormLabel>
                <Popover>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col">
                <FormLabel>Tanggal Jatuh Tempo Kalibrasi Berikutnya (Opsional)</FormLabel>
                <Popover>
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
