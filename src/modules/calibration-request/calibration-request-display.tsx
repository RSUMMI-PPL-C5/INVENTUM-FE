...
export default function CalibrationRequestDisplay() {
	...
	const updateURLParams = (
		...
		router.push(`/dashboard/calibration-request?${params.toString()}`, {
			scroll: false,
		});
	};
	...
	const fetchCalibrationRequests = async () => {
		try {
			...
			if (!response.ok) {
				toast.error(
					<>
						Error fetching calibration requests:
						<br />
						{result.message}
					</>
				);
			...
			setCalibrationRequests(result.data);
			setPaginationMeta(result.meta);
		} catch (error) {
			console.error("Error fetching calibration requests:", error);
			toast.error(
	...
	const handleDelete = async () => {
		...
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_API_URL}/request/calibration/${requestToDelete}`,
				{
					method: "DELETE",
					headers: {
						Authorization: token ? `Bearer ${token}` : "",
			...
			if (!response.ok) {
				toast.error(
					<>
						Error deleting calibration request:
						<br />
						{result.message}
					</>
				);
			...
			console.error("Error deleting calibration request:", error);
			toast.error(
				error instanceof Error
					? error.message
					: "Error deleting calibration request"
	...
	useEffect(() => {
		...
		if (success === "create") {
			setTimeout(
				() => toast.info("Permintaan kalibrasi berhasil dibuat"),
				100
			);
		}
		if (success === "delete") {
	...
	return (
		<div className="space-y-6 font-plus-jakarta-sans">
			...
			<DeleteDialog
				...
				title="Hapus Permintaan Kalibrasi"
				description="Apakah Anda yakin ingin menghapus permintaan kalibrasi ini? Tindakan ini tidak dapat dibatalkan."
