...
export default function CalibrationRequestDisplay() {
  ...
  const fetchCalibrationRequests = async () => {
    try {
      setLoading(true);
      const token = Cookies.get("token");
      const queryParams = buildQueryParams(filters);
      let url = `${process.env.NEXT_PUBLIC_API_URL}/request/calibration`;
      ...
      if (!response.ok) {
        ...
        console.log(res);
        throw new Error("Failed to fetch calibration requests");
      }

      const data = await response.json();
      setCalibrationRequests(data.data);
    } catch (err) {
      console.error("Error fetching calibration requests:", err);
      toast.error("Gagal memuat permintaan kalibrasi");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (requestId: string) => {
    if (!confirm("Apakah Anda yakin ingin menghapus permintaan kalibrasi ini?")) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Gagal menghapus permintaan kalibrasi");
      }

      fetchCalibrationRequests();
      toast.info("Permintaan kalibrasi berhasil dihapus");
    } catch {
      toast.error("Gagal menghapus permintaan kalibrasi");
    }
  };
  ...
  useEffect(() => {
    ...
    if (success === "create") {
      setTimeout(() => toast.info("Permintaan kalibrasi berhasil dibuat"), 100);
    }
    if (success === "delete") {
      setTimeout(() => toast.info("Permintaan kalibrasi berhasil dihapus"), 100);
    }
    hasRun.current = true;
  }, [searchParams]);
  ...
  const navigateToRequestEdit = (requestId: string) => {
    router.push(`/dashboard/requests/calibration/${requestId}/edit`);
  };

  const navigateToRequestDetail = (requestId: string) => {
    router.push(`/dashboard/request/calibration/${requestId}`);
  };

  const navigateToRequestCreate = () => {
    router.push(`/dashboard/request/calibration/create`);
  };

  return (
    <div className="space-y-6 font-plus-jakarta-sans">
      <h1 className="text-header-h5 font-bold font-poppins">Permintaan Kalibrasi</h1>
        ...
        <div className="flex flex-col gap-6 py-6 px-6">
          <div className="space-y-2">
            ...
            <p className="text-s-medium">
              Kelola, pantau, dan atur semua permintaan kalibrasi alat medis
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        <div className="relative w-full">
          ...
          <Input
            type="text"
            placeholder="Cari permintaan kalibrasi..."
            value={search}
      ...
      {loading && (
        <div className="flex justify-center p-8">
          <div className="animate-pulse text-center">
            Memuat Permintaan Kalibrasi...
          </div>
        </div>
      )}

      {/* Calibration Request Table */}
      {!loading && (
        <div className="border rounded-lg overflow-hidden">
          <Table data-testid="calibration-requests-table">
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {calibrationRequests.length > 0 ? (
                ...
                <TableRow>
                  ...
                  >
                    {search
                      ? "Tidak ada permintaan kalibrasi yang cocok dengan pencarian Anda"
                      : "Tidak ada permintaan kalibrasi yang ditemukan"}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
