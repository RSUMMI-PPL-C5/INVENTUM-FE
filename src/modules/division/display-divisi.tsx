...
import { ChevronRight, ChevronDown, Loader2, Pencil, Trash2 } from "lucide-react"
...
export default function DisplayDivisi() {
  ...
  async function fetchDivisionChildren(divisionId: number) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to fetch children for division ${divisionId}`)
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to delete division. It might have child divisions.",
        variant: "destructive",
      })
    } finally {
      setIsDeleting(false)
  ...
  const renderDivisionTree = (divisions: Division[]) => {
    if (!divisions || divisions.length === 0) {
      return <div className="text-sm text-gray-500 p-2">No divisions found</div>
    }
            ...
            <div className="flex items-center p-2 rounded-md hover:bg-gray-100 cursor-pointer justify-between">
              ...
              <div className="flex space-x-2">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={(e) => handleEditDivision(division, e)}
                >
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={(e) => handleDeleteClick(division, e)}
                >
            ...
            {expandedDivisions[division.id] && (
              <div className="ml-4 border-l border-gray-200 pl-2 mt-1">
                {loadingChildren[division.id] ? (
                  ...
                  <div className="text-sm text-gray-500 p-2">No subdepartments</div>
                )}
              </div>
            )}
          </div>
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <span className="text-header-h5 font-bold font-poppins">Struktur Divisi</span>
      ...
      <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete the division &quot;{divisionToDelete?.divisi}&quot;?
              {divisionToDelete?.children?.length ? (
                <span className="text-red-500 block mt-2">
                  Warning: This division has sub-divisions that will also be deleted.
                </span>
              ) : null}
            </AlertDialogDescription>
          </AlertDialogHeader>
