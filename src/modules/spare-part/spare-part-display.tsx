...
export default function SparepartDisplay() {
  ...
  const fetchSpareparts = async () => {
    try {
      ...
      const url = `${process.env.NEXT_PUBLIC_API_URL}/spareparts?${apiParams.toString()}`
      ...
      if (responseData.data && Array.isArray(responseData.data)) {
        ...
        console.warn("API response doesn't have the expected structure with data and meta fields")
      }
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/spareparts/${sparepartToDelete}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
    ...
    <div className="space-y-6">
      ...
      <div className="bg-primary-solid items-center p-2 flex gap-3 h-fit text-white rounded-lg overflow-hidden">
        ...
        <div className="flex flex-col gap-6 py-6 px-6">
          <div className="space-y-2">
            <h2 className="text-header-h6 font-bold font-poppins">Suku Cadang</h2>
            <p className="text-s-medium">
              Kelola informasi suku cadang untuk peralatan medis, termasuk stok, lokasi, dan riwayat penggunaan.
            </p>
          </div>
          <Button variant="ghost" className="w-fit" onClick={() => navigateToCreate()}>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4">
        ...
        <div className="w-full sm:w-auto">
          <FilterDialog onApplyFilter={handleApplyFilter} currentFilters={searchParams} />
        </div>
      </div>
      ...
      {loading && (
        <div className="flex justify-center p-8">
          <div className="animate-pulse text-center">Memuat Suku Cadang...</div>
        </div>
      )}
