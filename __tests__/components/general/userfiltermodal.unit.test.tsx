...
describe("UserFilterModal", () => {
  ...
  it("doesn't render if not opened", () => {
    render(
      <UserFilterModal isOpen={false} filters={mockFilters} onConfirm={mockOnConfirm} onCancel={mockOnCancel} />
    );
  ...
  it("allows selecting a date range", () => {
    ...
    const modifiedOnStart = screen.getAllByPlaceholderText("Tanggal Mulai")[1];
    ...
    const modifiedOnEnd = screen.getAllByPlaceholderText("Tanggal Akhir")[1];
