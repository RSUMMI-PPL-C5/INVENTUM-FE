...
jest.mock("@/modules/division/display-divisi", () => {
  return jest.fn(() => <div data-testid="display-divisi">DisplayDivisi Component</div>)
})
...
describe("DisplayDivisi Component", () => {
  ...
  it("should render loading state initially", async () => {
    ...
    // Check loading state
    expect(screen.getByTestId("loading-state") || screen.getAllByRole("skeleton")[0]).toBeInTheDocument()
    
    // Wait for content to load
    await waitFor(() => {
      expect(screen.queryByTestId("loading-state") || screen.queryAllByRole("skeleton")[0]).not.toBeInTheDocument()
    })
  })
  ...
  it("should navigate to edit page when edit button is clicked", async () => {
    ...
    const editButtons = screen.getAllByRole("button").filter(btn => 
      btn.innerHTML.includes('svg') && !btn.innerHTML.includes('Loader')
    )
  ...
  it("should handle API error when deleting division", async () => {
    ...
    // Verify error handling
    await waitFor(() => {
      expect(screen.getByText(/Failed to delete division/i)).toBeInTheDocument()
    })
  })
